...
//        is_null($cat_id) ? $query = Content::find()->all() : $query = $query = Content::find()->where(['cat_id' => $cat_id]);
        ...
        $models_pop = Content::find()->where('content_date > DATE(NOW()) - INTERVAL 3 MONTH')->limit(5)->orderBy(['hits'=> SORT_DESC])->all();
    ...
    {
        $model = $this->findModel($id);
        if (\Yii::$app->user->can('editContent', ['content'=>$model])) {
...
//        }
    ...
    {
        $model = $this->findModel($id);
        if (\Yii::$app->user->can('deleteContent', ['content'=>$model])) {
